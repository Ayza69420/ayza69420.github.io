document.getElementById("button").addEventListener("click",() => {
    ...
    for (const person of d) {
        ...
        for (const k of possibilities.slice(1)) {
            ...
            if (!(diff_a == diff_b)) {
                possibilities.splice(possibilities.indexOf([possibilities[0], k][Math.max(...[diff_a, diff_b])==diff_b], 1))
            }
                ...
                possibilities.splice([possibilities[0], k][Math.max(...[diff_c, diff_d])==diff_c],1)
            }
        }
